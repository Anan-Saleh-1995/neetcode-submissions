class Solution {
    ...
    */
    ...
    // i was wrong about reverse theres no need to at all
    
    // public String[] reverseStack(String[] logs) {
    //     int left = 0;
    //     int right = logs.length - 1;
    //     String[] reversedLogs = Arrays.copyOf(logs, logs.length); //O(N) space
    //     while(left < right) { // scoped change its not leaving here....
    //         String temp = reversedLogs[left];
    //         reversedLogs[left] = reversedLogs[right];
    //         reversedLogs[right] = temp;
    //         System.out.println("left" + " " + reversedLogs[left] + " " + "right" + " " + reversedLogs[right]);
    //         left++;
    //         right--;
    //     }
    //     for (int i = 0; i < reversedLogs.length; i++) {
    //         System.out.println(reversedLogs[i]);
    //     }
    //     return reversedLogs;
    // }
    ...
    public int minOperations(String[] logs) {
        ...
        //     right--;
        // }
       // String[] newLog = reverseStack(logs); // Time Complexity O(N) Space Complexity O(1)
        int toRoot = 0;
